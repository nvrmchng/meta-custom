...
Sidebar::Sidebar()
{
    ...
    connect(controlwindow, SIGNAL(latest_control_value(QByteArray)), this, SLOT(write_control_update(QByteArray)));
    connect(settingwindow, SIGNAL(latest_setting_parameter(QByteArray)), this, SLOT(write_setting_update(QByteArray)));
}
